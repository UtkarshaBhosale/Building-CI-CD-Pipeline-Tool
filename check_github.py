...
def get_latest_commit_sha():
    ...
    try:
        ...
        print(f"[ERROR] Failed to fetch latest commit: {e}")
        return None
    except KeyError:
...
def read_stored_commit_sha():
    ...
    if os.path.isfile(LAST_COMMIT_FILE):
        try:
            ...
            print(f"[ERROR] Failed to read stored commit file: {e}")
    return None
...
def write_commit_sha(sha):
    ...
    try:
        ...
        print(f"[ERROR] Failed to write commit SHA: {e}")
...
def main():
    ...
    if latest_sha != stored_sha:
        print("[INFO] New changes detected.")
